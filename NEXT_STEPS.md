...
>
> ## 🆕 UPDATE 2026-07-06 (later) — admin-controlled entitlements + production wiring (7 phases)
> Landed on `main` (`1497ff6 · e20a44a · 46ae7e0 · c7f29a0 · 597ea04 · 89ab077 · d9aa5ae · 8b4b447`; `tsc` 0 · `build` 392/392 · `eslint` 0 · adversarial review 0 findings). This makes every paid feature **admin-grantable without a gateway** (user files an `entitlement_request` → admin approves → `grantEntitlement` writes the real subscription/credits/wallet row → existing `access-control.ts` unlocks it) and wires the mock admin console + content pages + a real Blog CMS. **Authoritative doc: [`ENTITLEMENTS_AND_WIRING_BUILD_LOG.md`](./ENTITLEMENTS_AND_WIRING_BUILD_LOG.md)** (build log + apply/operate runbook). **New pending migrations:** `20260706_entitlement_requests.sql`, `20260706_draft_cart_payload.sql`, `20260706_content_and_ops.sql`, `20260706_articles_seed.sql`. Still deferred: real gateway, Academy LMS, sector dashboards, i18n.
