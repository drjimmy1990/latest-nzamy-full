...
> 🔴 **حاجز 2026-09-18:** الخطوات الحية ومسارات Windows أدناه تاريخية، وليست تعليمات تشغيل للتسليم الجديد. `library:reseed:wipe` يرفض فورًا؛ `library:reseed -- --input <المكتبة> --output <مجلد-جديد-خارجي>` يقوم بالتحليل والبذر **الجاف فقط** ولا يتصل بقاعدة للتحقق. لا تشغّل `library:seed` أو `library:clear --live` هنا. اقرأ README جذر حزمة التسليم وعقد Staging/النسخ الاحتياطي قبل أي قاعدة. يتطلب `library:parse` خرجًا جديدًا صريحًا أيضًا.
