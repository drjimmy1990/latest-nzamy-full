...
> 🔴 **تصحيح حاكم لحزمة التيست 2026-09-18:** هذه الوثيقة تاريخية في أوامرها الحية أدناه. اتبع README الجذر في حزمة التسليم. `library:reseed:wipe` **محظور ويُرفض قبل أي عملية**؛ كان يمسح قاعدة حية قبل بذر صار يرفض التشغيل بلا نمط صريح، ويعارض حظر المسح في `supabase/one-time/2026-09-02_clear_test_data.sql`. `library:reseed` الآن **تحليل ثم بذر جاف فقط**؛ يلزم `--input` و`--output` جديد خارج المستودع/المدخل، ولا يختبر DB/API. `library:parse` يلزم له `--output` جديد كذلك. لا تنفّذ أمثلة `library:seed` أو `library:clear --live` أو أوامر قاعدة من هذه النسخة؛ قرار النشر/Staging والنسخ الاحتياطي عقد منفصل. فواحِص drift تحتاج مسارات صريحة بدل مسار `D:\` القديم. الأقسام التالية محفوظة كمرجع تاريخي غير تنفيذي ولا تُعد إذنًا لمسح أو كتابة.
