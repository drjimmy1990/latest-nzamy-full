...
function parseHijriDateToValue(dateStr?: string): number {
  if (!dateStr) return 0;
  ...
  if (parts.length === 3) {
    ...
    const yMatch = clean.match(/\b(13|14)\d{2}\b/);
    if (yMatch) {
      return parseInt(yMatch[0]) * 10000;
    }
  }
  return 0;
}
...
export default function LegalLibraryPage() {
  ...
  const searchSuggestions = nq.length >= 2 ? (() => {
    ...
    // Inject realistic mockup data for search demonstration
    if (nq.includes(normalizeArabic("بطلان"))) {
      results.push(
        { label: "بطلان العقد لعدم الأهلية في نظام المعاملات المدنية", type: "laws", typeLabel: isRTL ? "نظام" : "Law", id: "civil-transactions" },
        { label: "بطلان إجراءات القبض والتفتيش لعدم الحصول على إذن مسبق", type: "precedents", typeLabel: isRTL ? "مبدأ" : "Principle" },
        { label: "بطلان الشرط الجزائي المبالغ فيه في عقود المقاولات الإدارية", type: "precedents", typeLabel: isRTL ? "سابقة" : "Precedent" },
        { label: "بطلان حكم التحكيم الصادر بالمخالفة لأحكام الشريعة الإسلامية والنظام العام", type: "precedents", typeLabel: isRTL ? "مبدأ" : "Principle" }
      );
    } else if (nq.includes(normalizeArabic("سقوط"))) {
      results.push(
        { label: "سقوط الحق في إقامة دعوى الضمان بفوات المدة النظامية", type: "laws", typeLabel: isRTL ? "نظام" : "Law", id: "civil-transactions" },
        { label: "سقوط الخصومة لعدم السير فيها بفعل المدعي لمصلحة المدعى عليه", type: "precedents", typeLabel: isRTL ? "مبدأ" : "Principle" },
        { label: "سقوط عضوية مجلس الإدارة في الشركات المساهمة للغياب المتكرر", type: "laws", typeLabel: isRTL ? "نظام" : "Law", id: "companies-law" }
      );
    }

    // Scan systems
    const FULL_LAWS_SYSTEMS = [
      { id: "sys-1", slug: "civil-transactions", title: "نظام المعاملات المدنية", desc: "القانون المدني السعودي الشامل المنظم للعقود والالتزامات والمسؤولية التقصيرية والممتلكات.", free: true },
      { id: "sys-2", slug: "commercial-courts", title: "نظام المحاكم التجارية", desc: "تنظيم اختصاصات وإجراءات المحاكم التجارية ونظر الدعاوى والطلبات المتعلقة بالتجار.", free: true },
      { id: "sys-3", slug: "labor-law", title: "نظام العمل السعودي", desc: "تنظيم العلاقة التعاقدية بين صاحب العمل والعامل وحقوق الطرفين والنزاعات العمالية.", free: false },
      { id: "sys-4", slug: "companies-law", title: "نظام الشركات الجديد", desc: "تنظيم شركات المساهمة والمحدودة والتضامن وحوكمتها وقواعد الاندماج والاستحواذ.", free: false }
    ];

    FULL_LAWS_SYSTEMS.forEach(s => {
  ...
  const sortedLaws = [...filteredLaws].sort((a, b) => {
    if (precSort === "year-desc" || precSort === "date-desc") {
      return parseHijriDateToValue(b.lastUpdated) - parseHijriDateToValue(a.lastUpdated);
    }
    if (precSort === "year-asc") {
      return parseHijriDateToValue(a.lastUpdated) - parseHijriDateToValue(b.lastUpdated);
    }
    return 0;
  });
  ...
  const sortedOrders = [...filteredOrders].sort((a, b) => {
    if (precSort === "year-desc" || precSort === "date-desc") {
      return parseHijriDateToValue(b.date) - parseHijriDateToValue(a.date);
    }
    if (precSort === "year-asc") {
      return parseHijriDateToValue(a.date) - parseHijriDateToValue(b.date);
    }
    return 0;
  });
                    ...
                    <div className={`p-4 border-b ${isDark ? "border-white/[0.06] bg-white/[0.02]" : "border-gray-100 bg-slate-50/50"} text-right`}>
                      <div className="flex items-center justify-between mb-2">
                        <span className={`text-xs font-black ${isDark ? "text-white" : "text-gray-900"}`}>
                          {q.includes("بطلان")
                            ? "تم العثور على ١,٤٢٠ نتيجة لكلمة (بطلان)"
                            : q.includes("سقوط")
                            ? "تم العثور على ٨٤٠ نتيجة لكلمة (سقوط)"
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-2">
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${isDark ? "bg-[#C8A762]/10 text-[#C8A762]" : "bg-amber-50 text-amber-800"}`}>
                          مبادئ وسوابق ({q.includes("بطلان") ? "١٢٨٠" : q.includes("سقوط") ? "٧٤٠" : "٥"})
                        </span>
