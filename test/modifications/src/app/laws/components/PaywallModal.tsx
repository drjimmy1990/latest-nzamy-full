...
export const PLANS = [
  ...
  },
  {
    ...
    price: "٣٠٠", priceEn: "300", period: "لـ ٣ أشهر", periodEn: "/ 3 months",
    desc: "٣ شهور بخصم ٥٠% وعليهم ٣ شهور لـ ٣ زملاء من اختيارك", descEn: "3 months with 50% off + 3 free months for 3 colleagues of your choice",
    features: [
      "كل مزايا خطة النصوص",
      "المبادئ القضائية المستقرة",
      "السوابق القضائية الكاملة",
      "ربط الأنظمة بأحكامها القضائية",
      "٣ تراخيص إضافية مجانية للزملاء",
      "خصم ٥٠٪ لفترة محدودة",
    ],
    featuresEn: [
      ...
      "3 additional free licenses for colleagues",
...
export function AdvancedSearchModal({ isOpen, onClose, isRTL, isDark, onApplySearch }: {
  isOpen: boolean; onClose: () => void; isRTL: boolean; isDark: boolean;
  ...
  const handleApply = () => {
    if (onApplySearch && searchRef.current) {
      onApplySearch(searchRef.current.value);
    } else {
      onClose();
    }
  };
  ...
  const ISSUERS = [
    ...
    { v: "ncnp",    ar: "المركز الوطني لتنمية القطاع غير الربحي",        en: "NCNP" },
  ];
  const BRANCHES = [
    { v: "",           ar: "الكل",                              en: "All" },
