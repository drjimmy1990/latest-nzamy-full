...
function DraftItem({ e, isSelected, onToggle, onRemove, isDark, isRTL }: { e: CartEntry; isSelected: boolean; onToggle: () => void; onRemove: () => void; isDark: boolean; isRTL: boolean }) {
  ...
  const copyArticle = () => {
    ...
    const selectedText = getSelectedTextWithin(`draft-article-${e.articleId}`, e.articleText);
    const textBlock = selectedText || e.articleText;
  ...
  const copyReg = () => {
    if (!e.execReg) return;
    const selectedText = getSelectedTextWithin(`draft-reg-${e.articleId}`, e.execReg.text);
    const textBlock = selectedText || e.execReg.text;
  ...
  return (
    <div className={`rounded-xl border overflow-hidden ${isDark ? "border-white/[0.06] bg-zinc-800/50" : "border-slate-100 bg-slate-50"}`}>
      ...
      <div className="flex items-center gap-2 px-3 pt-3 pb-1">
        ...
        />
        <div className="flex-1 flex items-center gap-1.5 min-w-0 cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
          {e.isArticleAdded ? (
            <>
              {/* green article badge */}
        ...
        <button onClick={() => setIsExpanded(!isExpanded)} className={`p-1 rounded-lg flex-shrink-0 transition text-gray-400 hover:text-gray-200`}>
          {isExpanded ? <CaretUp size={11} /> : <CaretDown size={11} />}
        </button>
      </div>

      {/* ─ article text preview / full text ─ */}
      {e.isArticleAdded && (
        isExpanded ? (
          <p id={`draft-article-${e.articleId}`} className={`px-3 pb-2.5 text-[11px] leading-relaxed whitespace-pre-wrap select-text ${isDark ? "text-zinc-300" : "text-slate-600"}`}>
            {e.articleText}
          </p>
        ) : (
          <p className={`px-3 pb-2 text-[10px] line-clamp-1 cursor-pointer ${isDark ? "text-zinc-500" : "text-slate-400"}`} onClick={() => setIsExpanded(true)}>
            {e.articleText.split("\n")[0]}
          </p>
        )
      )}
      ...
      {hasExecReg && e.execReg && (
        <div className="mx-3 mb-2 flex flex-col gap-1.5">
          <div className={`px-2.5 py-2 rounded-lg flex items-center gap-2 ${isDark ? "bg-[#C8A762]/8 border border-[#C8A762]/20" : "bg-amber-50 border border-amber-200"}`}>
          ...
          {isExpanded && (
            <p id={`draft-reg-${e.articleId}`} className={`px-2.5 py-2 rounded-lg text-[10.5px] leading-relaxed whitespace-pre-wrap select-text border ${
              isDark ? "bg-black/10 border-white/[0.04] text-zinc-400" : "bg-amber-50/20 border-amber-100 text-amber-900"
            }`}>
              {e.execReg.text}
            </p>
          )}
