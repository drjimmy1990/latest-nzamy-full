...
> **Key framing:** the tester tested **live production**, which still runs the **old code** — our fixes are committed but **not deployed**. So "FIXED_BY_US" means *the code is fixed and will show once deployed*, not that nezamy.sa shows it today.
...
| `LAWYER-6.1` | تسريب بيانات التواصل (جوال/بريد/موقع) في الملف العام | Lawyer / profile | src/app/dashboard/lawyer/profile/page.tsx:326-328 still renders Phone/Envelope/Globe (phone/email/website) publicly; profile fix made data real but did not hide contact fields. |
| `LAWYER-6.3` | أزرار تالفة: 'تصدير PDF' معطل و'تعديل' يوجه لصفحة بيضاء | Lawyer / profile | Our commit added a dual-allowlist PATCH backend + reads real data, but the /dashboard/lawyer/profile/edit EDIT-FORM UI is on our DEFERRED list (still a dead link), and 'تصدير PDF' export was never wired. Both remain broken. |
