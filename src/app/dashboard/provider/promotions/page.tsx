...
const INITIAL_LINKS: ProviderPromoLink[] = [
  {
    id: "promo-prov-1",
    slug: "transl-2026",
    fullUrl: "https://nzamy.com/promo/transl-2026",
    providerId: "prov-555",
    providerName: "مكتب الترجمة المعتمد",
    providerType: "provider",
    promoType: "fixed_off",
    value: 50,
    targetService: "service-translation",
    targetServiceLabel: "ترجمة وثيقة قانونية",
    maxUses: 100,
    usedCount: 45,
    startsAt: "2026-05-01",
...
export default function ProviderPromotionsPage() {
  ...
  const [service, setService] = useState("ترجمة وثيقة قانونية");
  ...
  function handleCopy(url: string) {
    ...
    setToast("تم نسخ الرابط! يمكنك مشاركته الآن.");
  }
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className={`fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg z-50 p-6 shadow-2xl ${card}`}>
              ...
              <div className="space-y-4">
                <label className="block space-y-1">
                  ...
                  <div className="flex" dir="ltr">
                    ...
                    <input 
                      ...
                      className={`flex-1 rounded-r-xl border px-3 py-2 outline-none font-mono text-sm ${isDark ? "bg-[#0d1117] border-white/10 text-white" : "bg-white border-gray-200 text-gray-900"}`}
                    />
                  </div>
                </label>
                ...
                <label className="block space-y-1">
                  ...
                  <select value={service} onChange={e => setService(e.target.value)} className={`w-full rounded-xl border px-3 py-2 outline-none text-sm ${isDark ? "bg-[#0d1117] border-white/10 text-white" : "bg-white border-gray-200 text-gray-900"}`}>
                    <option value="ترجمة وثيقة قانونية">ترجمة وثيقة قانونية</option>
                    <option value="مراجعة عقد صياغة">مراجعة عقد صياغة</option>
                  </select>
                </label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <label className="block space-y-1">
                    ...
                    <select value={promoType} onChange={e => setPromoType(e.target.value as any)} className={`w-full rounded-xl border px-3 py-2 outline-none text-sm ${isDark ? "bg-[#0d1117] border-white/10 text-white" : "bg-white border-gray-200 text-gray-900"}`}>
                  ...
                  {promoType !== "free" && (
                    <label className="block space-y-1">
                      ...
                      <input type="number" value={value} onChange={e => setValue(Number(e.target.value))} className={`w-full rounded-xl border px-3 py-2 outline-none text-sm ${isDark ? "bg-[#0d1117] border-white/10 text-white" : "bg-white border-gray-200 text-gray-900"}`} />
                    </label>
                  )}
                </div>
