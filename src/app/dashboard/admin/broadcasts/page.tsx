...
import { Megaphone, Plus, CheckCircle, Clock, Users, PencilSimple, Trash, PaperPlaneTilt, Tray, WarningCircle } from "@phosphor-icons/react";
...
export default function AdminBroadcastsPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      // Honest failure state — no mock fallback. Zero rows renders the
      // page's empty state below (loadError distinguishes "failed to load"
      // from "genuinely no broadcasts yet").
      setItems([]);
      setLoadError(true);
    }
  }, []);
  ...
  const sendBroadcast = async (id: number | string) => {
    if (typeof id !== "string") return; // guard: Broadcast.id is DB uuid (string); defensive only
    try {
    ...
    <div className={`${bg} min-h-screen`} dir="rtl">
      <div className="max-w-3xl mx-auto p-4 md:p-8 space-y-6">
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className={`${card} p-4 shadow-sm space-y-3`}>
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <option value="individual">الأفراد / العملاء</option>
        ...
        <div className="space-y-3">
          {list.length === 0 && (
            <div className={`${card} p-10 shadow-sm flex flex-col items-center text-center gap-2`}>
              {loadError ? (
                <WarningCircle size={30} weight="thin" className={muted} />
              ) : (
                <Tray size={30} weight="thin" className={muted} />
              )}
              <p className={`text-sm font-bold ${isDark ? "text-gray-300" : "text-gray-700"}`}>
                {loadError ? "تعذر تحميل رسائل البث" : "لا توجد رسائل بث بعد"}
              </p>
              <p className={`text-xs ${muted}`}>
                {loadError ? "تحقق من الاتصال وحاول مرة أخرى" : "أنشئ أول رسالة بث لمستخدمي المنصة"}
              </p>
            </div>
          )}
          {list.map((b, i) => {
