...
export async function GET() {
  try {
    ...
    const monthlyRevenue = payments
      .filter((p: { status: string; created_at: string }) => p.status === "paid" && p.created_at >= thisMonthStart)
      .reduce((sum: number, p: { amount: number }) => sum + (p.amount ?? 0), 0);
    ...
    return NextResponse.json({
      ...
      pendingInvoices: payments.filter((p: { status: string }) => p.status === "requires_payment").length,
    });
  } catch (err) {
