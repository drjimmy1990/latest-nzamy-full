...
export async function GET() {
  ...
  const { data: requests, error: reqError } = await supabase
    ...
    .select("sender_id, status, type, created_at")
    .eq("receiver_id", uid);
  ...
  for (const req of requests) {
    const existing = clientMap.get(req.sender_id) || { requestCount: 0, activeCount: 0, lastActivity: "" };
    ...
    clientMap.set(req.sender_id, existing);
  }
  ...
  const { data: profiles } = await supabase
    .from("profiles")
    .select("id, full_name, email, phone, avatar_url, user_type")
    .in("id", clientIds);
  ...
  const clients = (profiles ?? []).map((profile) => {
    ...
    return {
      id: profile.id,
      name: profile.full_name || "عميل نظامي",
      email: profile.email,
      phone: profile.phone,
