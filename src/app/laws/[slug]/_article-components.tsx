...
export function MD({ text, isDark, isRTL = true }: { text: string; isDark: boolean; isRTL?: boolean }) {
    ...
    <div className="space-y-1.5">
      {text.trim().split("\n").map((line, i) => {
        ...
        if (line.startsWith("أ-") || line.startsWith("ب-") || line.startsWith("ج-") ||
          ...
          return <p key={i} className={`text-[13px] leading-relaxed ${listIndent} ${muted}`} dangerouslySetInnerHTML={{ __html: html }} />;
        if (line.trim() === "") return <div key={i} className="h-1" />;
        return <p key={i} className={`text-[13px] leading-relaxed ${muted}`} dangerouslySetInnerHTML={{ __html: html }} />;
      })}
    </div>
  );
}
...
function PrecedentCard({ pr, isDark, inCart, onToggle, locked, onLock, isRTL = true }: {
  ...
  const handleCopy = () => {
    if (locked) { onLock(); return; }
    navigator.clipboard.writeText(`${pr.court} - ${pr.caseNum} - ${pr.year}\n${pr.summary}`);
    setCopied(true); setTimeout(() => setCopied(false), 1600);
  };
  return (
    <div className={`rounded-xl border p-3 transition ${isDark ? "border-white/[0.06] bg-zinc-800/40" : "border-slate-100 bg-slate-50"}`} dir={isRTL ? "rtl" : "ltr"}>
...
// ─── Article Block ─────────────────────────────────────────────────────────────
export function ArticleBlock({ article, lawName, isDark, entry, onAddArticle, onRemoveArticle, onAddExecReg, onRemoveExecReg, onActive, isActive, showPaywall, onExplain, isRTL = true }: {
  ...
  const handleCopy = useCallback(() => {
    ...
    navigator.clipboard.writeText(`**${article.num} من ${lawName}**\n\n${article.text}`);
    setCopied(true); setTimeout(() => setCopied(false), 1800);
  }, [article, isLocked, showPaywall, lawName]);

  const handleCopyReg = useCallback(() => {
    if (isLocked) { showPaywall(); return; }
    if (!article.executiveReg) return;
    navigator.clipboard.writeText(`**${article.executiveReg.ref}**\n\n${article.executiveReg.text}`);
    setCopiedReg(true); setTimeout(() => setCopiedReg(false), 1800);
  }, [article, isLocked, showPaywall]);
  ...
  return (
    <motion.div
      ...
      className={`rounded-2xl border shadow-sm overflow-hidden cursor-pointer transition-colors ${isDark ? "bg-zinc-900" : "bg-white"} ${cardBorder}`}
    >
      {/* Header */}
      <div className={`flex items-center gap-2 px-4 py-2.5 border-b ${isDark ? "border-white/[0.05] bg-zinc-800/50" : "border-slate-100 bg-slate-50/80"}`}
        ...
        <span className={`text-[11px] font-black px-2.5 py-1 rounded-lg flex-shrink-0 ${isDark ? "bg-[#0B3D2E] text-[#C8A762]" : "bg-[#0B3D2E] text-white"}`}>
          {article.num}
        </span>
        ...
        <button onClick={handleCopy} className={`p-1.5 rounded-lg transition flex-shrink-0 ${isDark ? "hover:bg-white/[0.06] text-zinc-500" : "hover:bg-slate-100 text-slate-400"}`}>
          {copied ? <Check size={12} className="text-emerald-500" /> : <Copy size={12} />}
        </button>
        <button
          onClick={() => inCart ? onRemoveArticle(article.id) : (isLocked ? showPaywall() : onAddArticle(article))}
          className={`p-1.5 rounded-lg transition flex-shrink-0 ${inCart ? isDark ? "bg-red-900/20 text-red-400" : "bg-red-50 text-red-500" : isDark ? "bg-[#C8A762]/10 text-[#C8A762]" : "bg-amber-50 text-amber-600"}`}
        >
        ...
        {isRepealed ? (
          ...
          <>
            {isLocked ? (
              ...
              <MD text={article.text} isDark={isDark} isRTL={isRTL} />
            )}
            
            {/* Executive Reg — always visible */}
            {article.executiveReg && (
              <div className={`p-3 rounded-xl ${isRTL ? "border-r-2" : "border-l-2"} border-[#C8A762] ${isDark ? "bg-[#C8A762]/5" : "bg-amber-50/60"}`}>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  {/* Copy + Add buttons — always visible */}
                ...
                <MD text={article.executiveReg.text} isDark={isDark} isRTL={isRTL} />
              </div>
            )}

            {/* Cart additions for this article */}
            {entry && (entry.principles.length > 0 || entry.precedents.length > 0) && (
            ...
            {/* Amendments toggle */}
            {isAmended && article.amendments && article.amendments.length > 0 && (
              <div>
...
export function PreambleBlock({ text, isDark, isRTL = true }: { text: string; isDark: boolean; isRTL?: boolean }) {
  ...
  return (
    <div className={`rounded-2xl border overflow-hidden ${isDark ? "bg-zinc-900 border-white/[0.07]" : "bg-white border-slate-200 shadow-sm"}`}>
      <button type="button" onClick={() => setOpen(!open)} className={`w-full flex items-center gap-3 px-4 py-3 transition ${isDark ? "hover:bg-white/[0.03]" : "hover:bg-slate-50"}`}>
        ...
        <span className={`flex-1 ${textStart} text-[12px] font-bold ${isDark ? "text-zinc-300" : "text-zinc-700"}`}>{isRTL ? "ديباجة النظام والمرسوم الملكي" : "Preamble and Royal Decree"}</span>
      ...
      <AnimatePresence>
        {open && (
          <motion.div key="preamble-content" initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
            <div className={`px-4 pb-4 border-t text-[12px] leading-loose whitespace-pre-wrap ${isDark ? "border-white/[0.05] text-zinc-400" : "border-slate-100 text-slate-600"}`}>{text}</div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
